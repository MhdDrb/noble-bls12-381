...
let bls;
const G2_VECTORS = fs
  .readFileSync(join(__dirname, 'bls12-381-g2-test-vectors.txt'), 'utf-8')
  .trim()
  .split('\n')
  .map((l) => l.split(':'));
...
run(async () => {
  ...
  await mark('init', 1, () => {
    bls = require('..');
    p1 =
      bls.PointG1.BASE.multiply(
        0x28b90deaf189015d3a325908c5e0e4bf00f84f7e639b056ff82d7e70b6eede4cn
      );
    p2 =
      bls.PointG2.BASE.multiply(
  ...
  await mark('sign', 20, async () => await bls.sign('09', priv));
